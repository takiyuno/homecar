...
$objPHPExcel->getActiveSheet()->getStyle('A1:BA2')->getAlignment()->setWrapText(true);
			// Set alignments จัดรูปแบบหน้า ตรงกลางซ้ายขวากึ่งกลาง
			//echo date('H:i:s') . " Set alignments\n";
$objPHPExcel->getActiveSheet()->getStyle('A1:BA2')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$objPHPExcel->getActiveSheet()->getStyle('A1:BA2')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$objPHPExcel->getActiveSheet()->getStyle('A1:BA2')->getAlignment()->setWrapText(true);
...
$objPHPExcel->getActiveSheet()->getStyle('A1:BA2')->applyFromArray(
	array(
		'font'    => array(
			'bold'      => true
		),
...
->orderBy('data_cars.SendCar_Date', 'ASC')
->get();
foreach($data2 as $d_cus){
$user = DB::table('users')->where('username',$d_cus->Name_Saleplus)->first();
	...
	$objPHPExcel->getActiveSheet()->setCellValue('AJ'.($r+2),'=SUM(Z'.($r+2).':AI'.($r+2).')');
	$objPHPExcel->getActiveSheet()->setCellValue('AK'.($r+2),'=X'.($r+2).'-'.'AJ'.($r+2));
